...
probType **newProb(oneProblem *orig, stocType *stoc, timeType *tim, vector lb, double TOLERANCE) {
    ...
    for ( t = 0; t < tim->numStages; t++) {
        ...
        if ( !(prob[t]->sp->lp = setupProblem(prob[t]->sp->name, prob[t]->sp->type, prob[t]->sp->mac,
                                              prob[t]->sp->mar, prob[t]->sp->objsen, prob[t]->sp->objx,
                                              prob[t]->sp->rhsx, prob[t]->sp->senx, prob[t]->sp->matbeg,
                                              prob[t]->sp->matcnt, prob[t]->sp->matind, prob[t]->sp->matval,
                                              prob[t]->sp->bdl, prob[t]->sp->bdu, NULL, prob[t]->sp->cname,
                                              prob[t]->sp->rname, prob[t]->sp->ctype)) ) {
...
////
    ...
    for (t=0; t<tim->numStages; t++) {
        prob[t]->sp->lp = setupProblem(prob[t]->sp->name, prob[t]->sp->type, prob[t]->sp->mac, prob[t]->sp->mar, prob[t]->sp->objsen,
                                       prob[t]->sp->objx, prob[t]->sp->rhsx, prob[t]->sp->senx, prob[t]->sp->matbeg, prob[t]->sp->matcnt,
                                       prob[t]->sp->matind, prob[t]->sp->matval, prob[t]->sp->bdl, prob[t]->sp->bdu, NULL, prob[t]->sp->cname,
                                       prob[t]->sp->rname, prob[t]->sp->ctype);
    ...
    if ( prob ) {
        for ( t = 0; t < T; t++ ) {
            if (prob[t]) {
                ...
                if (prob[t]->Bbar) {
                    for (n=0; n < T; n++) {
                        if ( prob[t]->Bbar[n] ) freeSparseMatrix(prob[t]->Bbar[n]);
                    }
                    mem_free(prob[t]->Bbar);
                }
